...
- [x] 验证：`flutter analyze` 改动文件 0 问题；`flutter test test/widgets/word_dictionary_sheet_test.dart test/providers/tts/tts_controller_dict_prewarm_test.dart` 17 passed。
  ...
  **完成时间**: 2026-07-02
## 已完成：订阅平台灰度（仅 iOS 启用订阅 + AI 限额按平台生效）

Google Play 未配置、macOS IAP 未验证，订阅先只在 iOS 上线。两个独立开关：**购买/UI 侧**=编译期 RC key 注入（不给某平台 key 即停用，订阅入口全链路隐藏）；**限额侧**=后端 env 平台列表（改 env 即时生效，无需发版）。未启用平台继续不限 AI 次数。

**客户端（`payment`）**
- [x] `revenuecat_config.dart`：`revenueCatApiKey` 仅 iOS 返回 Apple key（macOS 显式排除，IAP 验证后恢复 `|| Platform.isMacOS`）；新增 `isSubscriptionSupported`（= useLocalStoreKit || isRevenueCatConfigured）。macOS 上 RC 不初始化、`purchaseServiceProvider` 自动回退 Stub，零额外分支。
- [x] `subscription_availability.dart`（新）：`subscriptionAvailabilityProvider`——UI 门控单一入口，测试可 override。
- [x] 三处门控：设置页订阅 tile 不渲染；`openPaywall` 拦截 + SnackBar「当前平台暂不支持订阅」（兜底覆盖转录/意群/词典全部撞墙点）；`PaywallScreen` 渲染占位页（防 deep link/调试入口）。ARB 新增 `premiumUnavailableOnPlatform`（en/zh）。
- [x] `client_info.dart`（新）：`clientPlatformName()`/`clientInfoHeaders()`；`SentenceAiApiClient`/`TranscriptionApiClient` BaseOptions 恒带 `x-app-platform` + `x-app-version`（版本经 provider 读 `packageInfoProvider`，未 override 时降级省略，§7.18 原则）。
- [x] 测试：`client_info_test`（映射+headers 组装+两 client 携带）、`subscription_availability_test`（openPaywall 拦截/放行）、`paywall_screen_test` 补占位页用例、`settings_screen_test` 补入口隐藏用例（harness 默认 override available=true）。

**后端（`../fluency-frontend`）**
- [x] `packages/payments/libs/usage.ts`：新增 env `AI_QUOTA_ENFORCED_PLATFORMS`（逗号平台列表，直接读 process.env 同总开关惯例，默认空=不对任何平台限额）；`isFreeQuotaExceeded` 加第三参 `clientPlatform`，判定顺序：总开关 → 平台列表 → 订阅豁免 → 用量比较。**缺 header/非法值 fail-open**（老客户端无 402 处理，限了直接报错）。
- [x] 5 个 AI 端点透传 `request.headers.get('x-app-platform')`。`recordApiUsage` 不动（所有平台照记）。
- [x] 测试：`usage-quota-entitlement.test.ts` 扩为 13 例（平台矩阵：未设/不在列表/null/非法值/大小写空白/多平台/含非法项）全绿；既有 25 failed 为基线既有问题（stash 对比确认）。

**上线口径**（`docs/subscription-setup.md` §4/§7 已更新）
- 启用 iOS 限额：新版客户端铺开后，后端设 `AI_QUOTA_ENFORCEMENT_ENABLED=true` + `AI_QUOTA_ENFORCED_PLATFORMS=ios`。
- 后续 Android：Play 配置 → 注入 Google key 发版 → env 列表加 `android`；macOS：Phase 4 验证 → 恢复 config 分支 → 列表加 `macos`。

  **完成时间**: 2026-07-03

## 已完成：订阅豁免（RevenueCat → Supabase 同步，后端 `fluency-frontend`）

免费额度按「用户+功能+自然月」后端裁决（各 5 次超额 402），但此前后端无订阅状态，会把订阅用户也限到 5 次。本次接入 RC webhook 单向同步订阅状态到 Supabase，在额度校验前放行订阅用户。客户端不改（它本就直接读 RC CustomerInfo）。

**后端（`../fluency-frontend`）**
- [x] `packages/database/schema.ts`：新增 `user_entitlements` 表——**每 (用户, entitlement) 一行**（复合唯一键 `(user_id, entitlement_id)`，对齐 RC 官方 Supabase 集成做法，支持一个用户多个权益）；列 `user_id` FK→`auth.users` cascade、`entitlement_id` NOT NULL、`is_active`、`product_id`、`store`、`expires_at_ms`(bigint，避无时区 timestamp 时区坑)、`environment`、`last_event_id`、`last_event_at_ms`、`raw`(jsonb，事件原文——审计+逃生舱，避免为新字段频繁改表)、时间戳。复合唯一键前缀即覆盖 user_id 查询，无冗余索引。迁移 `0037_fluffy_william_stryker.sql`（仅建表+FK+唯一约束，无 CREATE SCHEMA）。
- [x] `packages/payments/libs/entitlements.ts`（新）：`hasActiveEntitlement`（用户**任一**行 `is_active && (expires_at_ms 空 || > now)`，expires 兜底自然过期/退款）；`applyRevenueCatEvent`（按 `entitlement_ids` 逐行 upsert、授予/撤销分类、TRANSFER 让出方全权益失效、匿名/非 UUID/无 entitlement 跳过）。**健壮性**：幂等（复合唯一键 upsert）+ 抗乱序/并发（DB 层原子 `setWhere` 只在 `last_event_at_ms ≤ 本事件` 时更新，无读改写竞态）+ 错误区分（FK 违约=永久错误→200 不重试；瞬时故障→抛出→webhook 500→RC 重投不丢事件）。
- [x] `packages/payments/libs/usage.ts`：`isFreeQuotaExceeded` 开头 `if (await hasActiveEntitlement) return false`（5 个 AI 端点零改动全放行）；更新 `kFreeMonthlyLimits` 注释。
- [x] `apps/app/app/api/revenuecat/webhook/route.ts`（新）：`Authorization` 共享密钥（`REVENUECAT_WEBHOOK_AUTH`）逐字比对 → 转 `applyRevenueCatEvent`；skipped 也回 200（不让 RC 无限重试），未知异常 500。
- [x] `packages/payments/keys.ts`：加可选 `REVENUECAT_WEBHOOK_AUTH`（未配置时 webhook 拒绝、不拖垮其余路由）；加总开关 `AI_QUOTA_ENFORCEMENT_ENABLED`（`string→bool` transform 只认 `'true'`，**默认关闭**）。
- [x] **限额总开关**：`isFreeQuotaExceeded` 首行 `if (!keys().AI_QUOTA_ENFORCEMENT_ENABLED) return false`——默认关闭时所有人无限用（供订阅功能铺开前上线其它功能），置 `'true'` 才启用限额；懒调 `keys()`（不在模块顶层，避免 5 路由 import 期触发全 payments env 校验）。
- [x] 测试：`apps/app/__tests__/`——`entitlements.test.ts`(12)、`revenuecat-webhook.test.ts`(6)、`usage-quota-entitlement.test.ts`(3) 共 21 例全绿；vitest.config 加 `server-only` 空桩别名（顺带修复此前因 server-only 抛错的既有 v2-auth 测试，零回归——基线对比 25 failed 不变、全在 8 个既有红文件内）。biome 改动文件 0 error。
- [ ] **待用户执行**：注入 `REVENUECAT_WEBHOOK_AUTH`、`pnpm db:migrate` 应用到线上 Supabase、RC 后台配置 webhook（URL+Auth 头，Sandbox+Prod）+ Sandbox 端到端验证（见 `docs/subscription-setup.md` §7）。

  **完成时间**: 2026-07-01

## 已完成：AI 免费额度后端裁决（每功能每月 5 次，超额弹订阅）

此前 `kAiTrialLimits` 全为 0 → 未订阅用户一点 AI 就直接弹订阅（还没试用就撞墙）。改为：额度裁决上移到后端，按「用户+功能+自然月」计数，满 5 次返回 402，客户端捕获后才弹订阅。**暂当所有用户为免费**（后端还不知道订阅状态，订阅豁免留待接入 RC webhook 后再加）。**5 个 AI 功能全部限额**（含词典 dictionary，2026-07-01 补齐）。

**后端（`../fluency-frontend`）**
- [x] `packages/payments/libs/usage.ts`：新增 `kFreeMonthlyLimits`（各 5）、`getMonthlyUsageCount`、`isFreeQuotaExceeded`（读当月计数 ≥ 上限即超额）。
- [x] 5 个端点（dictionary/translate/analyze/sense-groups/submit-transcription）认证后加额度校验：超额返回 `402 {code:'quota_exceeded'}`，先校验后干活，成功才 record（→ 恰好 5 次成功后第 6 次 402）。
- [x] 已在 `kFreeMonthlyLimits` 注释标注：**上线前必须先接入订阅状态、放行订阅用户**，否则订阅用户也会被限到 5 次。

**客户端（`payment`）**
- [x] `free_allowance_policy.dart`：`freeAllowancePolicyProvider` 改 `AlwaysAllowPolicy`（不再本地预判额度，放行发起请求，由后端裁决）。保留 `TrialAllowancePolicy` 供订阅阶段复用。
- [x] `sentence_ai_provider.dart`：新增 `_callWithQuotaMapping`，把后端 402 统一映射为 `AiFeatureQuotaExceededException`（翻译/解析/意群三处 L3 调用）；UI 既有 catch → 弹订阅。
- [x] `transcription_task_provider.dart`：新增 `TranscriptionQuotaExceeded` 状态，submit 返回 402 时进入；`manage_subtitles_sheet.dart` 监听到该状态 → 清状态 + `openPaywall`。
- [x] 词典（2026-07-01 补）：`lookup_controller.dart` 新增 `LookupQuotaExceeded`（AI 源 402 时进入）；`ai_dict_result_view.dart` 该态渲染升级按钮；`word_dictionary_sheet.dart` `_handleUpgrade` → `openPaywall`。测试：`lookup_controller_test`（402→LookupQuotaExceeded）、`ai_dict_result_view_test`（渲染升级按钮 + 回调）。
- [x] 测试：`sentence_ai_provider_test`（402→额度异常、非 402 原样抛）；`transcription_task_provider_test`（402→QuotaExceeded 状态）；`free_allowance_policy_test`/`feature_access_provider_test` 仍绿。`flutter analyze` 改动文件 0 问题。

  **完成时间**: 2026-07-01

## 已完成：记录用户 AI API 使用次数（后端权威 + 客户端成功计数）

## 已完成：记录用户 AI API 使用次数（后端权威 + 客户端成功计数）

5 个消耗后端算力的 AI 功能（翻译/解析/意群/单词释义/转录）此前后端完全不记录 per-user 使用次数，客户端也只有点击计数（失败也算）。本次两套独立机制：后端按「用户+功能+自然月」权威计数（计费/配额真相，服务端缓存命中也算），客户端新增 AI「成功次数」本地累计（供日后评价/付费提醒，本次只记录不做弹窗）。

**后端（`../fluency-frontend`）**
- [x] `packages/database/schema.ts`：新增 `api_usage` 表（`user_id` FK→`authUsers` `onDelete: cascade`、`feature`、`period 'YYYY-MM'`、`count`），唯一键 `(user_id, feature, period)` + `user_id` 索引；`pnpm db:generate` 生成 `migrations/0036_tired_tag.sql`（仅建表+FK REFERENCES `auth.users`+索引，无 `CREATE SCHEMA`）。
- [x] `packages/payments/libs/usage.ts`：新增 `recordApiUsage(userId, feature)` —— 复用 `billing.ts` 的 `getCurrentPeriod`，`onConflictDoUpdate` 原子自增；写失败只 `console.error` 不抛，不阻断业务。
- [x] 5 个 v2 端点成功路径调用（`dictionary`/`translate`/`analyze` 各 3 处成功 return、`sense-groups` 3 处、`user-audio/submit-transcription` 1 处）；错误/校验失败分支不计。
- [x] 验证：`pnpm -F app typecheck` 仅剩 1 条与本次无关的 `.next` 陈旧缓存报错（引用已不存在的 `v1/ai/dictionary`）。
- [ ] **待用户执行**：`pnpm db:migrate` 应用到线上 Supabase（需线上凭据）。

**客户端（`payment`）**
- [x] `event_names.dart` + `usage_event.dart`：新增 `translationSucceeded`/`analysisSucceeded`/`senseGroupSucceeded`/`aiWordAnalysisSucceeded`（转录成功复用已有 `aiTranscriptionCompleted`）。
- [x] `usage_counters.dart`：新增 4 个 `*SuccessCount` 字段（构造/increment/copyWith/toJson/fromJson，旧存档缺字段回落 0）。
- [x] `annotation_content_view.dart`：翻译/解析/意群在**成功分支**（意群仅非空结果）记录成功事件，与既有点击计数解耦。
- [x] `lookup_controller.dart`：AI 词典源（`source is AiDictionarySource`）非空结果时记录 `aiWordAnalysisSucceeded`。
- [x] `usage_providers.dart`：新增只读 `usageCountersProvider`（本次不接 UI）。
- [x] 测试：`usage_tracker_test.dart` 补 4 事件计数/roundtrip/旧存档回落；`lookup_controller_test.dart` 补 AI 成功记录 / null 不记 / 异常不记 / 非 AI 源不记。`flutter analyze` 改动文件 0 问题；相关测试全通过。
