...
/// 所有修改即时生效（直接写回 Provider），设置仅对当次精听有效。
library;
...
void showIntensiveListenSettingsSheet({required BuildContext context}) {
  showModalBottomSheet<void>(
    ...
    builder: (context) => const _IntensiveListenSettingsSheet(),
  );
}

/// 精听设置面板（即时生效，无需确认按钮）
class _IntensiveListenSettingsSheet extends ConsumerWidget {
  const _IntensiveListenSettingsSheet();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final settings = ref.watch(
      intensiveListenPlayerProvider.select((s) => s.settings),
    );
